...
fun List<ResultData>.pairOf() = if(isEmpty()) "" else joinToString {it.bundleOf()}
...
fun List<ResultData>.toParameterSpecNull() = map { ParameterSpec.builder(it.name, it.type.toClassName().copy(nullable = true)).defaultValue("null").build() }
